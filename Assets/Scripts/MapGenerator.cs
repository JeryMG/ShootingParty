...
public class MapGenerator : MonoBehaviour
{
	...
	public void GenerateMap() {
		...
		Transform maskTop = Instantiate(NavMeshMaskPrefab, Vector3.forward *(mapSize.x + MaxmapSize.x)/4 * tileSize, Quaternion.identity, mapHolder) as Transform;
		maskTop.localScale = new Vector3(MaxmapSize.x,1,((MaxmapSize.y - mapSize.y)/2)*tileSize);
		
		Transform maskBottom = Instantiate(NavMeshMaskPrefab, Vector3.back *(mapSize.x + MaxmapSize.x)/4 * tileSize, Quaternion.identity, mapHolder) as Transform;
